...
Note that `Eye` and `Head` are direct subclasses of `DrawableCircle`, just as `Torso` and `LimbSegment` are subclasses of `DrawableRectangle`. These superclasses are omitted from this diagram to keep it neat.
